...
export const metadata = {
  ...
  description:
    "Residential popcorn ceiling removal in Lorne Park: dust-controlled workflow, Level 5 skim, stain-block primer and a bright paint-ready finish. Clear schedule, tidy crews, written warranty.",
  alternates: {
...
export default function Page() {
  ...
  if (miss && miss.neighborhoods) {
    for (let j = 0; j < miss.neighborhoods.length; j++) {
      if (miss.neighborhoods[j].slug === "lorne-park")
        hood = miss.neighborhoods[j];
    }
  }
  const signals = (hood && hood.signals) || (miss && miss.signals) || null;
  ...
  // Build nearby neighborhood links under the new route base
  const nearby =
    miss && miss.neighborhoods
      ? miss.neighborhoods.filter((n) => n.slug !== "lorne-park").slice(0, 8)
      : [];
    ...
    <div className="container-x mx-auto px-4 py-10">
      ...
      <header className="max-w-5xl">
        <div className="flex flex-col gap-4 md:grid md:grid-cols-[1fr,auto] md:items-start md:gap-6">
          ...
          {/* Right: CTAs (both blue via your .btn-cta) */}
      ...
      <section id="scope" className="prose max-w-none mt-10">
        ...
        <ol>
          ...
          <li>
            <strong>Repairs:</strong> Corner cracks, tape lifts, fixture moves,
            pot-light rings, previous patch edges and water stains.
          </li>
          <li>
            <strong>Level 5 Skim:</strong> Full-field skim; feathered through
            transitions and into returns/bulkheads.
          </li>
          <li>
            <strong>Raking-Light Inspection:</strong> Checked from windows and
            LEDs; re-skim until the surface reads flat.
          </li>
          <li>
      ...
      {/* LOCAL RESOURCES — Lorne Park & Mississauga (internal-only copy) */}
      <section id="resources" className="mt-10 card p-6 bg-white">
        ...
        <ul className="mt-3 space-y-2 text-gray-700 list-disc pl-5">
          <li>
            <strong>City guidance:</strong> interior finish work notes and when
            inspections may apply.
          </li>
          <li>
            <strong>Waste & recycling:</strong> drywall off-cuts, compound pails
            and paint handling in Peel Region.
          </li>
          <li>
          ...
          <li>
            <strong>Access & staging:</strong> plan driveway use, pathway
            protection and staging so daily routines remain simple.
          </li>
          <li>
            <strong>Neighbour courtesy:</strong> brief notices for short,
            noisier windows; walk-off mats and tidy end-of-day resets.
          </li>
        </ul>
        <p className="mt-3 text-gray-700">
          We coordinate small logistics that keep projects smooth: pet safety,
          kids’ nap times, school pick-ups, and room sequencing so key spaces
          remain usable. The goal is a cleaner, faster path to a modern, smooth
          ceiling.
        </p>
      </section>
      ...
      {/* Nearby Areas (internal linking within Mississauga, new route base) */}
      {nearby.length ? (
      ...
      {/* Bottom links (updated to new base path) */}
